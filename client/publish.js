var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
// Dieser Event-Listener wird ausgelöst, sobald das gesamte Dokument geladen ist.
document.addEventListener('DOMContentLoaded', function () {
    // Referenz auf das Element, in dem die Beiträge angezeigt werden.
    var postWindow = document.getElementById('Feedwindow');
    // Asynchrone Funktion, um Beiträge vom Server zu laden.
    var showTweets = function () { return __awaiter(_this, void 0, void 0, function () {
        var response, result, allpost, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 5, , 6]);
                    return [4 /*yield*/, fetch('http://localhost:4200/getPost', {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                        })];
                case 1:
                    response = _a.sent();
                    if (!response.ok) return [3 /*break*/, 3];
                    return [4 /*yield*/, response.json()];
                case 2:
                    result = _a.sent();
                    allpost = result.allpost;
                    // Für jeden Beitrag wird HTML generiert und zum postWindow hinzugefügt.
                    allpost.forEach(function (post) {
                        var postElement = "\n            <div class=\"dark:bg-gray-800 p-4 rounded-lg shadow-md mb-4 w-1/2 mx-auto\">\n              <div>\n                  <span class=\"font-semibold text-white\">".concat(post.username, "</span>\n              </div>\n              <p class=\"text-white\">").concat(post.content, "</p>\n              <div class=\"mt-2\">\n                  <button class=\"text-blue-500 hover:underline\">Like</button>\n                  <button class=\"text-gray-500 hover:underline ml-2\">Comment</button>\n              </div>\n            </div>\n          ");
                        postWindow.innerHTML += postElement;
                    });
                    return [3 /*break*/, 4];
                case 3:
                    console.error('Fehler beim Auslesen der Posts:', response.statusText);
                    _a.label = 4;
                case 4: return [3 /*break*/, 6];
                case 5:
                    error_1 = _a.sent();
                    console.error('Fehler beim Laden der Tweets:', error_1);
                    return [3 /*break*/, 6];
                case 6: return [2 /*return*/];
            }
        });
    }); };
    // Aufruf der showTweets-Funktion beim Laden der Seite.
    showTweets();
    // Event-Listener für den Post-Button, um einen neuen Beitrag zu erstellen.
    document.getElementById('postButton').addEventListener('click', function (event) { return __awaiter(_this, void 0, void 0, function () {
        var postMessage, jwtToken, response, error_2;
        var _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    event.preventDefault();
                    postMessage = document.getElementById('postTextarea').value;
                    jwtToken = (_a = localStorage.getItem('Token')) !== null && _a !== void 0 ? _a : '';
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, fetch('http://localhost:4200/createPost', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            // Sendet den Beitrag und das JWT-Token im JSON-Format.
                            body: JSON.stringify({ postMessage: postMessage, jwtToken: jwtToken }),
                        })];
                case 2:
                    response = _b.sent();
                    // Prüft, ob die Anfrage erfolgreich war.
                    if (response.ok) {
                        console.log('Post erfolgreich gespeichert');
                    }
                    else {
                        console.error('Fehler bei der Speicherung des Postes:', response.statusText);
                    }
                    return [3 /*break*/, 4];
                case 3:
                    error_2 = _b.sent();
                    console.error('Netzwerkfehler:', error_2);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    }); });
    // Funktion zum Löschen eines Beitrags
    function deletePost(postId) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/];
            });
        });
    }
    // Funktion zum Bearbeiten eines Beitrags
    function editPost(postId, newContent) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/];
            });
        });
    }
    // Hinzufügen von Event-Listenern für Bearbeiten- und Löschen-Buttons
    document.querySelectorAll('.editButton').forEach(function (button) {
        button.addEventListener('click', function () {
            var postId = button.getAttribute('data-post-id');
            var newContent = prompt('Neuer Inhalt:');
            if (postId && newContent) {
                editPost(postId, newContent);
            }
        });
    });
    document.querySelectorAll('.deleteButton').forEach(function (button) {
        button.addEventListener('click', function () {
            var postId = button.getAttribute('data-post-id');
            if (postId && confirm('Beitrag löschen?')) {
                deletePost(postId);
            }
        });
    });
});
